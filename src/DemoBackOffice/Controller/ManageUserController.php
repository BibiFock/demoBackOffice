...
class ManageUserController extends ManageController
{
    ...
    public function connect(Application $app)
    {
        ...
        $index->match(
            "/",
            'DemoBackOffice\Controller\ManageUserController::user'
        )->bind("manage.users");

        $index->match(
            "/edit/{id}",
            'DemoBackOffice\Controller\ManageUserController::userEdit'
        )->assert('id', '[0-9]+')
        ->value('id', '')
        ->bind("manage.users.edit");

        $index->match(
            "/save/{id}",
            'DemoBackOffice\Controller\ManageUserController::userSave'
        )->assert('id', '[0-9]+')
        ->value('id', '')
        ->bind("manage.users.save");

        $index->match(
            "/del/{id}",
            'DemoBackOffice\Controller\ManageUserController::userDel'
        )->assert('id', '[0-9]+')
        ->bind("manage.users.delete");
            ...
            try {
                ...
                if ($form->isValid()) {
                    ...
                    if ($user->isSuperAdmin()) { // super admin can only change is password
                        $user = $app['manager.user']->changePassword(
                            $user->id,
                            $datas['password']
                        );
                    } else {
                        $user = $app['manager.user']->saveUser(
                            $user->id,
                            $datas['username'],
                            $datas['password'],
                            $datas['right'],
                    ...
                    $app['session']->getFlashBag()->add(
                        'info',
                        'User '.$user->username.' '.($isNew ? 'created' : 'updated')
                    );
                    if ($isNew) {
                        return $app->redirect(
    ...
    public function user(Application $app, $ajax = false)
    {
        ...
        return $app['twig']->render('manage/user-list.html.twig', array(
            'readonly' => !$access->canEdit(),
            'users' => $users,
            'jsonNewUser' => json_encode(
                array(
                    'url'=> $app['url_generator']
                    ->generate('manage.users.edit')
                )
            ),
            'ajax' => $ajax,
        ));
