...
class ManageUserTypeController extends ManageController
{
    ...
    public function connect(Application $app)
    {
        ...
        $index->match(
            "/",
            'DemoBackOffice\Controller\ManageUserTypeController::userType'
        )->bind("manage.rights");

        $index->match(
            "/edit/{id}",
            'DemoBackOffice\Controller\ManageUserTypeController::userTypeEdit'
        )->value('id', '')
        ->assert('id', '[0-9]*')
        ->bind("manage.rights.edit");

        $index->match(
            "/save/{id}",
            'DemoBackOffice\Controller\ManageUserTypeController::userTypeSave'
        )->value('id', '')
        ->assert('id', '[0-9]*')
        ->bind("manage.rights.save");

        $index->match(
            "/del/{id}",
            'DemoBackOffice\Controller\ManageUserTypeController::userTypeDel'
        )->assert('id', '[0-9]+')
        ->bind("manage.rights.delete");
    ...
    public function userType(Application $app, $ajax = false)
    {
        ...
        return $app['twig']->render('manage/rights-list.html.twig', array(
            'rights' => $typeUsers,
            'jsonNewRights' => json_encode(
                array(
                    'url'=> $app['url_generator']->generate('manage.rights.edit')
                )
            ),
            'ajax' => $ajax,
    ...
    public function userTypeEdit(Application $app, Request $request, $id, $ajax = false)
    {
        ...
        for ($i = 0; $i < count($sections); $i++) {
            $form->add('section_'.$sections[$i]->id, 'choice',
                array(
                    'choices' => array(
                        AccessType::$FORBIDDEN => 'Forbidden',
                        AccessType::$READONLY => 'Readonly',
                        AccessType::$EDIT => 'Manage'
                    ),
                    'data' => $userType->getAccessToSection($sections[$i]->id)
                    ->type,
                    'disabled' => $readonly,
                )
            );
        ...
        $jsonSaveUserType = array(
            'url' => $app['url_generator']->generate(
                'manage.rights.save',
                array('id' => $userType->id)
            )
        );
        //handle form request
        if ($request->isMethod('POST') && !$error) {
            ...
            try{
                ...
                if ($form->isValid()) {
                    ...
                    //save right infos
                    $userType = $app['manager.rights']->saveUserType(
                        $userType->id,
                        $datas['name'],
                        $datas['description'],
                        $access,
                    ...
                    $app['session']->getFlashBag()->add(
                        'info',
                        'Rights '.$userType->name.' '.($isNew ? 'created' : 'updated')
                    );
                    if($isNew){
                        return $app->redirect(
    ...
    public function userTypeDel(Application $app, Request $request, $id)
    {
        ...
        if ($id != "" && $request->isMethod('POST')) {
            try {
                ...
                $userType = $app['manager.rights']->getUserTypeById($id);
                if ($userType->id != '') {
                    $app['manager.rights']->deleteUserType($userType);
                }
                $app['session']->getFlashBag()->add(
                    'info',
                    'Section '.$userType->name.' deleted'
                );
            } catch(Exception $e) {
                $app['session']->getFlashBag()->add(
                    'error',
