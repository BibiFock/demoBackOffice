...
class UserProvider implements UserProviderInterface
{
    ...
    public function deleteUser(User $user)
    {
        if ($user->isSuperAdmin()) {
            throw new Exception('You cannot delete root account');
        }
    ...
    public function changePassword($id, $password)
    {
        ...
        if ($user->id == $id) {
            ...
            $this->app['db']->executeQuery(
                ...
                array($user->id, $user->password)
            );
            return $user;
        } else {
            throw new Exception('unfound user');
        }
    }
    ...
    public function saveUser($id, $login, $password, $userType, $new = false)
    {
        ...
        if ($user->id != "" || $id != "") {
            if($new || ($user->id != "" && $user->id != $id )){
                throw new Exception('Username already used');
            }
            if($user->isSuperAdmin()){
                throw new Exception('This is super admin user, it cannot be updated');
            }
            $sql = <<<SQL
update user
set login_user=?, password_user=?, date_modification_user=?, id_type_user=?
where id_user=?
SQL;
            $params[] = $id;
        } else {
            ...
            $sql = <<<SQL
insert into user(
    date_creation_user, login_user, password_user, date_modification_user, id_type_user
) VALUES(?,?,?,?,?)
SQL;
        }
    ...
    {
        $stmt = $this->app['db']->executequery(
            'SELECT id_user, login_user, password_user, id_type_user, date_modification_user from user'
        );
        ...
        for ($i = 0; $i < count($users); $i++) {
            $user = $users[$i];
            $userType = $this->app['manager.rights']
                ->getUserTypeById($user['id_type_user']);
            $users[$i] = new User(
                $user['id_user'],
                $user['login_user'],
                $user['password_user'],
                $userType, $user['date_modification_user']
            );
        }
        return $users;
    }
    ...
    protected function loadUserTypeAccess(UserType $userType)
    {
        ...
        if ($userType->id > 0) {
            $accessType = $this->app['manager.rights']
                ->getUserTypeAccess($userType);
